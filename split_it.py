...
def asking():
    while True:
        try:
            ...
            if i in ["s", "start"]:
                cache, blend = ask_for_blend_and_sim()
                start(cache, blend)
                break
...
def ask_for_blend_and_sim():
    ...
    return cache_folder_path, blend_file_path

def start(cache_folder_path, blend_file_path):
    ...
    for folder in cache_folders:
        ...
        if number_of_files != 0:
            for file in folder_files:
                ...
                files_and_their_sizes.append({file: genericpath.getsize(f"{cache_folder_path}\\{folder}\\{file}")})

        # print(files_and_their_sizes)
            folders_and_their_files[rf"{cache_folder_path}\\{folder}"] = files_and_their_sizes
