...
def list_folders():
    try:
        ...
        # Show the file dialog and store the selected folder path
        variables.folder_name = filedialog.askdirectory()  # This line replaces the input function
        if not variables.folder_name:  # Check if the user canceled the dialog
            return
        # Removed the check and error_msg call
        return
...
def processing_folders():
    ...
    if not selected_folder:  # Check if the user canceled the selection
        print("No folder selected, exiting...")
        return

    variables.destination_base_folder = selected_folder  # Update the destination folder based on user selection
